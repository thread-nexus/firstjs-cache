...
class MemoryStorageAdapter {
    ...
    async get(key) {
        try {
            ...
            if (meta?.compressed && Buffer.isBuffer(value)) {
                try {
                    const decompressed = await (0, compression_utils_1.decompressData)(value, 'gzip');
    ...
    async delete(key) {
        try {
            ...
            if (existed) {
                ...
                this.stats.deletes++;
                this.stats.size = this.calculateTotalSize();
                (0, cache_events_1.emitCacheEvent)(cache_events_1.CacheEventType.DELETE, { key });
            }
            return existed;
        }
        catch (error) {
    ...
    async clear() {
        try {
            ...
            // Reset stats
            this.stats.size = 0;
            (0, cache_events_1.emitCacheEvent)(cache_events_1.CacheEventType.CLEAR, { entriesRemoved: count });
        }
        catch (error) {
