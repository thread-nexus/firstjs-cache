...
export class BatchOperationHandler {
    constructor() {
        this.batch = new Map();
        this.timer = null;
        this.maxBatchSize = CACHE_CONSTANTS.MAX_BATCH_SIZE;
 ...
 */
export async function setCacheValue(provider, key, value, options) {
    validateCacheKey(key);
    validateCacheOptions(options);
    const startTime = performance.now();
    try {
        // Compress if needed
        const { data: processedValue, compressed } = await compressIfNeeded(JSON.stringify(value), {
 ...
 * @param key - Cache key
 */
export async function getCacheValue(provider, key) {
    validateCacheKey(key);
    const startTime = performance.now();
    try {
 ...
 * @param key - Cache key
 */
export async function deleteCacheValue(provider, key) {
    validateCacheKey(key);
    try {
 ...
 * @param provider - Cache provider
 */
export async function clearCache(provider) {
    try {
 ...
 */
export async function getManyValues(provider, keys) {
    const startTime = performance.now();
    try {
        if (provider.getMany) {
 ...
 * @param options - Cache options
 */
export async function setManyValues(provider, entries, options) {
    const startTime = performance.now();
    try {
        if (provider.setMany) {
