 ...
 */
import { CacheError, CacheErrorCode } from '../utils/error-utils';
...
export class CacheManager {
    ...
    async get(key, options) {
        ...
        try {
            ...
            if (this.circuitBreaker.isOpen()) {
                throw new CacheError(CacheErrorCode.CIRCUIT_OPEN, `Circuit breaker open for provider ${provider.name}`);
            }
    ...
    validateKey(key) {
        if (!key || typeof key !== 'string') {
            throw new CacheError(CacheErrorCode.INVALID_KEY, 'Invalid cache key');
        }
        if (key.length > 250) {
            throw new CacheError(CacheErrorCode.KEY_TOO_LONG, 'Cache key exceeds maximum length');
        }
    }
    validateValue(value) {
        if (value === undefined) {
            throw new CacheError(CacheErrorCode.INVALID_ARGUMENT, 'Cannot cache undefined value');
        }
    }
    selectProvider(preferredProvider) {
        ...
        if (!provider) {
            throw new CacheError(CacheErrorCode.PROVIDER_ERROR, 'No cache provider available');
        }
        return provider;
    }
    handleError(operation, error, context) {
        const cacheError = error instanceof CacheError ? error :
            new CacheError(CacheErrorCode.UNKNOWN, error.message, { operation, ...context });
        const metrics = {
