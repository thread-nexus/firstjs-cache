...
export class RateLimiter {
    ...
    async checkLimit(operation, count = 1) {
        ...
        if (this.isLimited(entry, config, count)) {
            if (burstable) {
                ...
                throw createCacheError(CacheErrorCode.RATE_LIMIT_EXCEEDED, `Rate limit exceeded for operation: ${operation}`, { operation });
            }
        }
    ...
    async handleBurst(operation, count) {
        ...
        if (windowUsage + count > burstLimit) {
            throw new CacheError(CacheErrorCode.RATE_LIMIT_EXCEEDED, `Burst limit exceeded for operation: ${operation}`, { operation });
        }
    }
    async queueRequest(operation) {
        ...
        if (queue.length >= (config.queueSize || 1000)) {
            throw new CacheError(CacheErrorCode.RATE_LIMIT_EXCEEDED, `Queue full for operation: ${operation}`, { operation });
        }
        await new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                if (index !== -1) {
                    ...
                    reject(new CacheError(CacheErrorCode.TIMEOUT, `Queue timeout for operation: ${operation}`, { operation }));
                }
            }, 5000);
            queue.push(() => {
